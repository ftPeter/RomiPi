...
#include "romipi_gazebo/romipi_drive.h"

RomiPiDrive::RomiPiDrive()
  : nh_priv_("~")
{
  //Init gazebo ros Romi Pi node
  ROS_INFO("RomiPi Simulation Node Init");
  ROS_ASSERT(init());
}
...
bool RomiPiDrive::init()
{
  ...
  laser_scan_sub_  = nh_.subscribe("scan", 10, &RomiPiDrive::laserScanMsgCallBack, this);
  joint_state_sub_ = nh_.subscribe("joint_states", 10, &RomiPiDrive::jointStateMsgCallBack, this);

  return true;
}

void RomiPiDrive::jointStateMsgCallBack(const sensor_msgs::JointState::ConstPtr &msg)
{
  right_joint_encoder_ = msg->position.at(0);
}

void RomiPiDrive::laserScanMsgCallBack(const sensor_msgs::LaserScan::ConstPtr &msg)
{
...
void RomiPiDrive::updatecommandVelocity(double linear, double angular)
{
...
*******************************************************************************/
bool RomiPiDrive::controlLoop()
{
  static uint8_t romipi_state_num = 0;
  double wheel_radius = 0.033;
  double romipi_rotation = 0.0;

  romipi_rotation = (rotate_angle_ * turning_radius_ / wheel_radius);

  switch(romipi_state_num)
  {
    case GET_TB3_DIRECTION:
      if (direction_vector_[CENTER] > front_distance_limit_)
      {
        romipi_state_num = TB3_DRIVE_FORWARD;
      }

      if (direction_vector_[CENTER] < front_distance_limit_ || direction_vector_[LEFT] < side_distance_limit_)
      {
        priv_right_joint_encoder_ = right_joint_encoder_ - romipi_rotation;
        romipi_state_num = TB3_RIGHT_TURN;
      }
      else if (direction_vector_[RIGHT] < side_distance_limit_)
      {
        priv_right_joint_encoder_ = right_joint_encoder_ + romipi_rotation;
        romipi_state_num = TB3_LEFT_TURN;
      }
      break;

    case TB3_DRIVE_FORWARD:
      updatecommandVelocity(LINEAR_VELOCITY, 0.0);
      romipi_state_num = GET_TB3_DIRECTION;
      break;
...
{
  ros::init(argc, argv, "romipi_drive");
  RomiPiDrive romipi_drive;

  ros::Rate loop_rate(125);

  while (ros::ok())
  {
    romipi_drive.controlLoop();
    ros::spinOnce();
    loop_rate.sleep();
  }
