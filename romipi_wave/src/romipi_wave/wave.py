...
class WaveNode():
    ...
    def close(self):
        ...
        self.leave()
        time.sleep(1.0)
        self.current_wave = self.wave_manager.get_current_active_wave()
        self.broadcast_node.close()

    def __str__(self):
        return "WaveNode (%s) wave assigned (%d) current (%d), isActive (%s) with node_list [%s]" % (
                self.name, self.assigned_wave, self.wave_manager.get_current_active_wave(),
                str(self.is_active()), str(self.broadcast_node))
